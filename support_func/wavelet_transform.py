...
class WaveletScatteringTransform:
    def __init__(self, segment_duration, fs, J=8, Q=16, frontend="torch"):
        ...
        self.scattering = Scattering1D(
            J=self.J, shape=(self.T,), Q=self.Q, frontend=frontend, max_order=1
        ).to(self.device)
    ...
    def __call__(self, sample):
        ...
        grayscale_tensor = (grayscale_tensor - grayscale_tensor.min()) / (
            grayscale_tensor.max() - grayscale_tensor.min() + 1e-8
        )

        colormap = plt.cm.viridis  # Get the colormap
        tensor_rgb = colormap(grayscale_tensor.cpu().numpy())[
            ..., :3
        ]  # Convert to NumPy and extract RGB channels

        # Convert RGB NumPy array to PyTorch tensor
        tensor_rgb_torch = torch.tensor(
            tensor_rgb, dtype=torch.float32
        )  # Shape: (227, 227, 3)
        ...
        return {"image": tensor_rgb_torch, "label": label}  # shape: [num_scales, T']
