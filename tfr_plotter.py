...
def plot_examples(dataset, title, num_samples=11):
    ...
    hc_indices = np.array(
        [i for i, label in enumerate(labels) if label == 0][:num_samples]
    )

    if num_rows == 2:
        pd_indices = [
            i for i, label in enumerate(labels) if (label == 1 or label == 2)
        ][:num_samples]
    else:
        pdoff_indices = [i for i, label in enumerate(labels) if label == 1][
            :num_samples
        ]
    ...
    fig, axes = plt.subplots(
        num_rows,
        num_samples,
        figsize=(4 * num_samples, 3 * num_rows),
        sharex=True,
        sharey=True,
    )
