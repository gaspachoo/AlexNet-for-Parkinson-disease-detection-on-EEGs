...
from support_func.cleaning_algos import matlab_like_cleaning
...
def process_and_save(indices, dataset,transform):
    ...
    for i,idx in enumerate(indices):
        ...
        image = sample["eeg"]
        image_filtered = matlab_like_cleaning(image.T).T
...
def preload_dataset(mode,electrode_name,number_samples,save=False, medication=None):
    ...
    print("Applying WST")
    transform_wst = wt.WaveletScatteringTransform(number_samples, J=7, Q=24)
    ...
    if save == True:
        ...
        if "iowa" in folder_path:
            train_path = path + 'train_iowa_seg.pt'
            val_path = path + 'val_iowa_seg.pt'
        else:
            if medication == None:
                train_path = path + 'train_sd_onandoff.pt'
                val_path = path + 'val_sd_onandoff.pt'
            elif medication == "on":
                train_path = path + 'train_sd_on_seg.pt'
                val_path = path + 'val_sd_on_seg.pt'
            else :
                train_path = path + 'train_sd_off_seg.pt'
                val_path = path + 'val_sd_off_seg.pt'
...
if __name__ == "__main__":
    number_samples = 92160 #max for iowa : 60600, max for san_diego : 92160
    electrode_name='Fz' ## AFz for sd, Fz for iowa
